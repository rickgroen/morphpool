...
__global__ void unpool_forward_kernel(
...
torch::Tensor unpool_cuda_forward(
  ...
  // Initialize as zeros the output volume.
  torch::Tensor output = torch::zeros(torch::IntList{B, C, H_OUT, W_OUT}, torch::dtype(torch::kF32).device(torch::kCUDA, device));

  const int THREADS = 192;
  const int Z = (H_OUT * W_OUT + THREADS - 1) / THREADS;
  const dim3 blocks(B, C, Z);
  AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "unpool_cuda_forward", ([&] {
    unpool_forward_kernel<scalar_t><<<blocks, THREADS>>>(
